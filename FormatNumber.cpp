#ifndef ALS_UTILITIES_FORMAT_NUMBER_CPP
#define ALS_UTILITIES_FORMAT_NUMBER_CPP
...
int als::utilities::position_of_most_significant_digit(const long double x)
{
    ...
    {
        return (int) std::floor(std::log10(std::fabs(x)*(1 + epsilon)));
    }
}

long double als::utilities::long_double_pow10(const unsigned int exponent)
{
    static const long double pow10_array[309] =
    ...
    return pow10_array[exponent];
}

unsigned long long int als::utilities::long_pow10(const unsigned int exponent)
{
    static const unsigned long long int pow10_array[19] =
...
long double als::utilities::round_to_precision(const double x, const unsigned int precision)
{
    if (precision == 0)
    {
        return 0;
    ...
    {
        int exponent = (int) std::floor((x != 0) ? 
            std::log10(std::fabs(x)*(1 + 1./long_double_pow10(precision+1))) : 0);
        return (exponent - (int)precision + 1 >= 0) ?
            std::round(x / long_double_pow10(exponent - precision + 1)) * long_double_pow10(exponent - precision + 1)
            : std::round(x * long_double_pow10(precision - exponent - 1)) / long_double_pow10(precision - exponent - 1);
    }
}

long double als::utilities::ceil_to_precision(const double x, const unsigned int precision)
{
    if (precision == 0)
    {
        return 0;
    ...
    else
    {
        int exponent = (int) std::floor((x != 0) ?
            std::log10(std::fabs(x)*(1 + 1./long_double_pow10(precision+1))) : 0);
        return (exponent - (int)precision + 1 >= 0) ?
            std::ceil(x / long_double_pow10(exponent - precision + 1)) * long_double_pow10(exponent - precision + 1)
            : std::ceil(x * long_double_pow10(precision - exponent - 1)) / long_double_pow10(precision - exponent - 1);
    }
}

long double als::utilities::floor_to_precision(const double x, const unsigned int precision)
{
    if (precision == 0)
    {
        return 0;
    ...
    else
    {
        int exponent = (int) std::floor((x != 0) ?
            std::log10(std::fabs(x)*(1 + 1./long_double_pow10(precision+1))) : 0);
        return (exponent - (int)precision + 1 >= 0) ?
            std::floor(x / long_double_pow10(exponent - precision + 1)) * long_double_pow10(exponent - precision + 1)
            : std::floor(x * long_double_pow10(precision - exponent - 1)) / long_double_pow10(precision - exponent - 1);
    }
}

#endif // ALS_UTILITIES_FORMAT_NUMBER_CPP